...
每个实体拥有 10 种抗性值（3 物理 + 7 罪孽），范围 0.01 ~ 5.0，精确到 2 位小数。
...
| 砸鸡蛋                             | +1  |
| 治疗僵尸村民                          | +10 |
| 闪电使猪→僵尸猪灵/苦力怕→闪电苦力怕/哞菇→变异/村民→女巫 | +10 |
| 僵尸在水中转化为溺尸                      | +5  |
...
16. 受罪孽伤害/造成罪孽伤害 +1 碎片；击杀普通生物 +5；击杀玩家 +20
17. 手动触发消耗 = max(1, baseCost - 2 × enchantLevel)
18. 溢出阈值 500，即死阈值 1000
19. 玩家断线自动清理碎片数据
20. 任意罪孽碎片首次达到 500，总积（totalProduct）-0.1，最低降至 0.1

### 附魔规则

21. 罪孽附魔：RARE/5 级/主手/任意物品可用
22. 物理抗性附魔：RARE/4 级/四护甲槽/每级 -5% 伤害

### 网络规则

23. 抗性修改必须经过服务端归一化后再同步回客户端
24. 碎片触发由服务端验证消耗，客户端仅发送请求
25. 玩家加入时自动同步抗性 + 碎片数据
