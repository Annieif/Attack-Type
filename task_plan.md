...
### S2. 游戏内验证
- [ ] 验证 GUI 抗性描述实时更新（输入数值时描述立即变化）
- [ ] 验证退出重进后抗性配置和碎片数量保留
- [ ] 验证忧郁碎片速率（每10点伤害+1）
- [ ] 验证碎片≥100时自动Lv.3触发，<100时停止
- [ ] 验证总积衰减后抗性值实际变化
- [ ] 验证碎片数不再溢出为负数
...
| 罪孽 | 获取速率 | 评估 |
|------|---------|------|
| 暴怒 WRATH | 连杀 7/5/3/1，战斗时 ~30-50/min | ✅ 合理 |
| 色欲 LUST | 繁殖+1, 鸡蛋+1, 转化+5/+10 | ⚠️ 鸡蛋可刷 |
| 怠惰 SLOTH | AFK +1/min, 睡眠+5 | 🔴 过慢（建议提升至 +3/min） |
| 暴食 GLUTTONY | 进食 +1/次 | ✅ 合理 |
| 忧郁 GLOOM | +1/10伤害 | ✅ 修复后合理 |
| 傲慢 PRIDE | 1728物品+1, 成就+10 | 🔴 极慢（建议降至 9组=576） |
| 嫉妒 ENVY | +1/min | 🔴 过慢（建议提升至 +3/min） |

总积衰减：每次-0.1, 最多7次(7罪孽), 最低0.1 → ✅ 合理
溢出触发：≥100→Lv.3消耗100→<100停止→循环 → ✅ 合理
...
| # | 任务 | 说明 |
|---|------|------|
| 35a | 统一配置文件 | 将 SinFragmentData 常量、获取速率、阈值等迁移到 JSON 配置 |
...
| # | 任务 | 说明 |
|---|------|------|
| 36a | 服务端压力测试 | 多玩家同时操作碎片/抗性，检查并发问题 |
| 36b | 网络包优化 | 减少不必要的同步频率，合并碎片更新 |
| 36c | 权限系统 | 细化 OP 权限，区分管理员/玩家指令 |
...
| # | 任务 | 说明 |
|---|------|------|
| 37a | 粒子效果增强 | 不同罪孽等级使用不同粒子密度/大小 |
| 37b | 音效系统 | 触发罪孽/碎片溢出/即死时的音效反馈 |
| 37c | 屏幕震动 | 被罪孽攻击命中时的轻微屏幕震动 |
| 37d | 伤害数字 | 显示罪孽伤害类型和数值的浮动文字 |
...
| # | 任务 | 说明 |
|---|------|------|
| 38a | 自定义进度 | 首次触发罪孽、首次碎片溢出、首次即死等 |
| 38b | 进度奖励 | 完成进度给予碎片或抗性奖励 |
| 38c | 进度本地化 | zh_cn/en_us 翻译 |
