...
| 35 | 配置系统重构 | ✅ done | config.json + ModConfig + /attacktype reload + preset |
| 36 | 多人游戏适配 | ✅ done | 并发安全加固 + 网络包优化 + 权限系统 |
| 37 | 视觉与反馈增强 | ✅ done | 粒子效果增强 + 音效 + 屏幕震动 + 伤害数字 |
| 38 | 成就与进度系统 | ✅ done | 7 个进度 + 经验奖励 + 本地化 |
| 39 | 药水与状态效果系统 | ✅ done | 194 种药水变体 + 酿造配方 + 伤害计算集成 |
| 40 | Mod 兼容 | ⏳ pending | API 导出、DamageType 注册、兼容性测试 |
| 41 | 发布准备 | ⏳ pending | Modrinth/CurseForge 发布、宣传材料 |
| 42 | 性能优化 | ✅ done | EnumMap 查找表 + 代码去重 + 监听器合并 + Tick 降频 |
| 43 | 生物系统 | ⏳ pending | 自定义生物实体 + 罪孽属性生物 + 掉落表 |
| 44 | 护甲系统 | ⏳ pending | 罪孽护甲套装 + 护甲效果 + 材质资源 |
...
| 39c | 效果逻辑 | ✅ 强化/弱化/守护/易损/提升/降低接入伤害计算，爆发效果接入反伤 + 物理抗性降低 + 无视抗性穿透 |
...
### 阶段42 — 性能优化
**目标：** 确保大量实体/玩家时性能稳定，消除冗余代码。

| # | 任务 | 说明 |
|---|------|------|
| 42a | EnumMap 查找表 | ✅ ModStatusEffects 用 3 个 EnumMap 替代 42 分支 switch-case，O(1) 查找 |
| 42b | 药水注册去重 | ✅ ModPotions 抽取 registerPotionLevels() 消除 4 个注册方法的重复代码 |
| 42c | 监听器合并 | ✅ SinFragmentAcquisition 2 个 ALLOW_DAMAGE 监听器合并为 1 个 |
| 42d | Tick 降频 | ✅ Pride 傲慢碎片扫描从每 tick 降为每 100 tick（5 秒） |
| 42e | 效果集成 | ✅ MixinLivingEntity 集成 applySinEffects/applyPhysEffects/applyBurstThorns |
...
| # | 任务 | 说明 |
|---|------|------|
| 43a | 自定义生物 | 创建 7 种罪孽生物实体（每种罪孽 1 种），含 AI 行为 |
| 43b | 罪孽属性 | 生物自带罪孽属性攻击，掉落对应罪孽碎片 |
| 43c | 掉落表 | 生物掉落罪孽相关物品（碎片、药水材料等） |
...
| # | 任务 | 说明 |
|---|------|------|
| 44a | 护甲物品 | 7 种罪孽护甲套装（头盔/胸甲/护腿/靴子），含材质 |
| 44b | 护甲效果 | 穿戴时提供罪孽抗性加成或特殊效果 |
| 44c | 合成配方 | 使用罪孽碎片和材料合成护甲 |
