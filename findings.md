...
| # | 文档错误 | 实际代码 | 影响 |
|---|---------|---------|------|
| 1 | README "基础碎片获取" 整节 | 不存在于代码中 | 删除 |
| 2 | 忧郁 "每 1 点伤害" | GLOOM_DAMAGE_PER_FRAGMENT=10 | 修正为 "每 10 点" |
| 3 | 色欲闪电 "+10" | LUST_LIGHTNING_AMOUNT=5 | 修正为 "+5" |
| 4 | 怠惰AFK "+1" | SLOTH_AFK_AMOUNT=3 | 修正为 "+3" |
| 5 | 傲慢制作 "27组(1728)" | PRIDE_CRAFT_THRESHOLD=576 | 修正为 "9组(576)" |
| 6 | 嫉妒攀比 "+2" | ENVY_COMPARE_AMOUNT=3 | 修正为 "+3" |

**根本原因：** 阶段34进行了平衡性调整（S4），但 README 未同步更新。同时"基础碎片获取"是早期设计文档中的概念，实际实现中已移除。

### 验证通过

碎片消耗 40/70/100、伤害公式、溢出/即死阈值、激活持续、暴怒连杀、色欲(繁殖/鸡蛋/治愈/溺尸)、暴食、傲慢(成就/生产)、嫉妒目击 — 均与代码一致。
